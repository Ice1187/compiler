...
fn symbol_match(text: &mut String) -> Token::Token {
 ...
 //    let syb_str = syb_cap.as_str().to_string();
    ...
    let (_value, _type, syb_len)  = keysyb_check(&syb_str);
    token._value = _value.to_string();
    token._type = _type.to_string();
	text.replace_range(..syb_len, "");
    token
}
...
fn keysyb_check(syb: &str) -> (&str, &str, usize) {
    let syb = syb.trim();
    let syb = syb.to_string();
    if (syb == "<=") || (syb == ">=") || (syb == "&&") || (syb == "||") || (syb == "==") || (syb == "!=")  {
        match &syb[..2] {
            "<=" => ("<=", "LESS_EQUAL", 2),
            ">=" => (">=", "GREATER_EQUAL", 2),
            "&&" => ("&&", "LOGIC_AND", 2),
            ...
            "==" => ("==", "EQUAL", 2),
            "!=" => ("!=", "NOT_EQUAL", 2),
            _ => panic!("Lexer: Unrecognized len 2 symbol."),
        }
    }
    else {
        match &syb[..1] {
            ...
            ">" => (">", "GREATER", 1),
            "=" => ("=", "ASSIGN", 1),
            _ => panic!("Lexer: Unrecognized symbol"),
        }
    }
}
